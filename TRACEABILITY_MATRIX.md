...
| DPTR configuration | `6. Interrupt Style` | `USB_HUB_ARCHITECTURE.md §7` | High | Pointer corruption during ISR-side `xdata` access | Do not assume multiple DPTR support and do not introduce unsafe ISR-side pointer handling | Compiler config review, code review | Runtime validation on ISR and `xdata` paths |
...
| CFU handler definition and region | `10. Firmware Update` | `USB_HUB_ARCHITECTURE.md §2`, `§3` | Critical | Update path executes from unsafe or undefined region | Do not assume CFU handler meaning, ownership, or execution region | Linker review, memory map review | Update validation evidence |
...
| Over-current model | `4. USB Role` | `USB_HUB_ARCHITECTURE.md §10` | High | Wrong over-current reporting behavior | Do not assume global or per-port over-current behavior | Descriptor review, config review | Over-current behavior verification |
